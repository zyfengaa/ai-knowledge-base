...
**Black Forest Labs (2024) "FLUX: Rectified Flow Transformers"**
...
- **Esser et al. (2024) "Scaling Rectified Flow Transformers for High-Resolution Image Synthesis"** — SD3 提出的 MMDiT（双流多模态 Transformer）。如果你对 Transformer 架构的最新设计感兴趣，建议读 Section 3（MMDiT 设计），这是 2024 年的事实标准设计。

- **Jia et al. (2025) "D2iT: Dynamic Diffusion Transformer for Accurate Image Generation"** — 提出动态 DiT 架构。如果你对"DiT 在推理效率上的优化"感兴趣，建议阅读其动态 token 剪枝策略。
...
- **Gu et al. (2024) "DART: Denoising Autoregressive Transformer for Scalable Text-to-Image Generation"** — 将去噪与自回归结合的尝试。如果你对比 VAR 和 DART 的设计差异，可以发现"下一尺度"vs"下一 token"两条路线的区别。

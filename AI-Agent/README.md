...
| 模块 | 核心问题 | 设计思想 |
|------|---------|---------|
| **01-Agent 基础范式** | LLM 怎么从"只会说话"变成"会调用工具"？ | 用「输出结构化指令 → 解析执行 → 结果反馈」的循环替代纯文本生成——ReAct 是核心模式 |
...
| **05-工程框架与协议** | Agent 系统怎么设计、集成、标准化、评估？ | 用「统一协议（MCP）+ 可组合框架 + 标准化评估」替代各自为政——让 Agent 可插拔可观测 |
...
| 解决的问题 | 留下的麻烦 |
|-----------|-----------|
| Toolformer 让 LLM 自己学会决定"是否需要调用API"；ReAct 建立了 思考→行动→观察 循环范式；OpenAI Function Calling 使之产品化 | 单步工具调用，无法处理多步骤复杂任务；Agent 仍是"一次调用一次回应"，没有长期推理链；工具调用质量不稳定 |

### 3. 推理增强时代（2023）

| 解决的问题 | 留下的麻烦 |
|-----------|-----------|
| Tree-of-Thoughts 把单线推理扩展为多路径搜索；Reflexion 让 Agent 能从错误中反思并自我改进；Plan-and-Solve 让 Agent 先规划再执行 | 推理成本大幅上升（多次 LLM 调用）；Agent 仍是单线程的"一个人在战斗"；缺乏长期记忆导致每次从头推理 |

### 4. 多智能体 + 记忆爆发（2023-2024）

| 解决的问题 | 留下的麻烦 |
|-----------|-----------|
| Generative Agents 建立完整 Agent 框架（记忆流+反思+社交）；AutoGen 实现多 Agent 对话协作；MemGPT 赋予 Agent 长期记忆管理能力 | 多 Agent 行为不稳定、通信开销大；框架碎片化（各自为政）；Agent 评估困难——怎么才算"好 Agent"？ |

### 5. 工程标准化时代（2024）

| 解决的问题 | 留下的麻烦 |
|-----------|-----------|
| MCP 协议统一工具接口标准；LangChain/AutoGen/CrewAI 等框架成熟；AgentBench/WebArena 等评估基准出现 | 安全问题凸显（Prompt 注入/工具滥用）；Agent 自主性难以控制；幻觉在 Agent 场景中被放大 |
...
- **ReAct (Yao 2023)**：把思考链和行动交错，让 LLM 在"想"和"做"之间切换
...
| 范式 | 核心优点 | 核心代价 | 关键约束 |
|------|---------|---------|---------|
| **ReAct 循环**（思考→行动→观察） | 简单直观，容易实现和调试 | 线性执行，无法并行探索 | ✅ 大多数场景可用 |
...
### 3. 工具抽象层：从硬编码到 MCP 协议

| 阶段 | 年份 | 贡献 | 局限 |
|------|------|------|------|
| **硬编码工具** | ~2022 | 每个工具手写调用逻辑 | 不可复用，每项目重写 |
| **Function Calling** | 2023 | OpenAI 标准化工具描述格式 | 绑定单一模型，非通用 |
| **Toolformer 自学习** | 2023 | LLM 自己学会"什么场景调什么工具" | 自举训练复杂，不实用 |
| **MCP 协议** | 2024 | 工具即服务，统一注册/发现/调用接口 | 生态仍在建设，需要 Server 端适配 |

**MCP 协议是 2024 年至今的工具集成事实标准。** 核心思想：工具是独立服务，通过标准协议注册和调用，Agent 端和工具端解耦。
...
├── 1000~10000 条 → 用 ReAct 轨迹数据训练完整 Agent
└── > 10000 条 → 基于 RL 的 Agent 优化（工具调用奖励）
```

> Agent 领域的大多数实践落在"< 100 条"区间——所以目前主流是 Prompt Engineering + Function Calling，而非训练专用模型。Gorilla (2023) 证明了检索增强可以让 API 调用不依赖微调。
...
| 你已经熟悉的 | 你需要补齐的 |
|-------------|-------------|
| LLM 的文本生成和对话能力 | Agent 的感知-规划-行动循环（ReAct） |
| Prompt Engineering / CoT 等提示技巧 | 结构化工具调用（Function Calling / MCP） |
| RAG 基础概念 | 多 Agent 协作的通信和任务分配机制 |
| — | Agent 的安全风险和对齐挑战 |
...
```
1. **01-Agent 基础范式**——和你现有 LLM 知识距离最近，重点补 ReAct 循环 + Function Calling
   ↓
2. **02-推理与规划**——从"单步调用"到"多步推理"，理解 ToT 和 Reflexion
   ↓
3. **04-Agent 记忆与知识**——Agent 怎么记住过去、利用外部知识
   ↓
4. **03-多智能体系统**——从单 Agent 到多 Agent 协作
   ↓
5. **05-工程框架与协议**——MCP 协议 + 主流框架 + 评估工具
   ↓
...
- **Agent 评估体系缺失**：LLM 评估已经成熟（MMLU/GSM8K），但 Agent 评估需要"环境+任务+交互"的综合基准——目前 AgentBench/WebArena 覆盖场景有限，且难以评估长期行为。
...
| 模块 | 核心篇数 | 拓展篇数 | 核心论文 |
|------|---------|---------|---------|
| 01-Agent 基础范式 | 4 | 2 | Yao (2023) ReAct, Schick (2023) Toolformer, Patil (2023) Gorilla,  OpenAI (2023) Function Calling |
| 02-推理与规划 | 4 | 2 | Wang (2022) CoT-SC, Yao (2023) ToT, Shinn (2023) Reflexion, Wang (2023) Plan-and-Solve |
